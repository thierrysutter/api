...
include_once '../dao/DaoConvocation.php';
...
switch($request_method)
{
  case 'GET':
    ...
    else if(!empty($_GET["keywords"]))
    {
      ...
      RechercherConvocation($keywords);
    }
    else
    {
      getConvocations();
...
function getConvocations()
{
  try
  {
    ...
    // initialize object
    $convocation = new DaoConvocation($db);

    // query products
    $stmt = $convocation->getAll();
    $count = count($stmt);
    if($count > 0)
    {
...
function AjouterConvocation()
{
  try
  {
    ...
    if(!empty($data->competition))
    {
      ...
      $cat->setRencontre($data->rencontre);

      // create the product
      if($convocation->creerConvocation($cat))
      {
...
function SupprimerConvocation()
{
  try
  {
    ...
    if($convocation->supprimerConvocation($data->id))
    {
      // set response code - 200 ok
      http_response_code(200);
...
function ModifierConvocation()
{
  try
  {
    ...
    // create the product
    if($convocation->modifierConvocation($cat))
    {
      // set response code - 201 created
      http_response_code(201);

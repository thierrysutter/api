...
include_once '../dao/DaoParcours.php';
include_once '../dto/DtoParcours.php';
...
function getParcourss()
{
  try
  {
    ...
    // initialize object
    $parcours = new DaoParcours($db);

    // query products
    $stmt = $parcours->getAll();
    $count = count($stmt);
    if($count > 0)
    {
...
function AjouterParcours()
{
  try
  {
    ...
    if(!empty($data->club))
    {
      ...
      $cat = new DtoParcours(array());
      $cat->setClub($data->club);
      
      // create the product
      if($parcours->creerParcours($cat))
      {
...
function SupprimerParcours()
{
  try
  {
    ...
    if($parcours->supprimerParcours($data->id))
    {
      // set response code - 200 ok
      http_response_code(200);
...
function ModifierParcours()
{
  try
  {
    ...
    // create the product
    if($parcours->modifierParcours($cat))
    {
      // set response code - 201 created
      http_response_code(201);

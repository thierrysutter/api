...
include_once '../dao/DaoCompetition.php';
...
switch($request_method)
{
  case 'GET':
    ...
    else if(!empty($_GET["keywords"]))
    {
      ...
      RechercherCompetition($keywords);
    }
    else
    {
      getCompetitions();
...
function getCompetitions()
{
  try
  {
    ...
    // initialize object
    $competition = new DaoCompetition($db);

    // query products
    $stmt = $competition->getAll();
    $count = count($stmt);
    if($count > 0)
    {
...
function AjouterCompetition()
{
  try
  {
    ...
    if(!empty($data->libelle) && !empty($data->anneeDebut) && !empty($data->anneeFin))
    {
      // instantiate database and product object
      $database = new Database();
      $db = $database->getConnection();
      ...
      $cat = new DtoCompetition(array());
      $cat->setLibelle($data->libelle);
      $cat->setTypeCompetition($data->typeCompetition);
      $cat->setCategorie($data->categorie);
      $cat->setDivision($data->division);
      $cat->setSaison($data->saison);
      $cat->setEquipe($data->equipe);
      
      // create the product
      if($competition->creerCompetition($cat))
      {
...
function SupprimerCompetition()
{
  try
  {
    ...
    if($competition->supprimerCompetition($data->id))
    {
      // set response code - 200 ok
      http_response_code(200);
...
function ModifierCompetition()
{
  try
  {
    ...
    // create the product
    if($competition->modifierCompetition($cat))
    {
      // set response code - 201 created
      http_response_code(201);

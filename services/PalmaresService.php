...
include_once '../dao/DaoPalmares.php';
include_once '../dto/DtoPalmares.php';
...
function getPalmaress()
{
  try
  {
    ...
    // initialize object
    $palmares = new DaoPalmares($db);

    // query products
    $stmt = $palmares->getAll();
    $count = count($stmt);
    if($count > 0)
    {
...
function AjouterPalmares()
{
  try
  {
    ...
    if(!empty($data->texte))
    {
      ...
      $cat = new DtoPalmares(array());
      $cat->setAnnee($data->annee);
      $cat->setTexte($data->texte);
      
      // create the product
      if($palmares->creerPalmares($cat))
      {
...
function SupprimerPalmares()
{
  try
  {
    ...
    if($palmares->supprimerPalmares($data->id))
    {
      // set response code - 200 ok
      http_response_code(200);
...
function ModifierPalmares()
{
  try
  {
    ...
    // create the product
    if($palmares->modifierPalmares($cat))
    {
      // set response code - 201 created
      http_response_code(201);

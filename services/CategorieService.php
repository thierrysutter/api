...
include_once '../dao/DaoCategorie.php';
...
function getCategories()
{
  try
  {
    ...
    // initialize object
    $categorie = new DaoCategorie($db);

    // query products
    $stmt = $categorie->getAll();
    $count = count($stmt);
    if($count > 0)
    {
...
function AjouterCategorie()
{
  try
  {
    ...
    if(!empty($data->libelle) && !empty($data->anneeDebut) && !empty($data->anneeFin))
    {
      ...
      $cat = new DtoCategorie(array());
      $cat->setLibelle($data->libelle);
      $cat->setAnneeDebut($data->anneeDebut);
      $cat->setAnneeFin($data->anneeFin);
      
      // create the product
      if($categorie->creerCategorie($cat))
      {
...
function SupprimerCategorie()
{
  try
  {
    ...
    if($categorie->supprimerCategorie($data->id))
    {
      // set response code - 200 ok
      http_response_code(200);
...
function ModifierCategorie()
{
  try
  {
    ...
    // create the product
    if($categorie->modifierCategorie($cat))
    {
      // set response code - 201 created
      http_response_code(201);

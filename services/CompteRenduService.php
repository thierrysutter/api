...
include_once '../dao/DaoCompteRendu.php';
...
switch($request_method)
{
  case 'GET':
    ...
    else if(!empty($_GET["keywords"]))
    {
      ...
      RechercherCompteRendu($keywords);
    }
    else
    {
      getCompteRendus();
...
function getCompteRendus()
{
  try
  {
    ...
    // initialize object
    $compteRendu = new DaoCompteRendu($db);

    // query products
    $stmt = $compteRendu->getAll();
    $count = count($stmt);
    if($count > 0)
    {
...
function AjouterCompteRendu()
{
  try
  {
    ...
    if(!empty($data->rencontre))
    {
      ...
      $cat = new DtoCompteRendu(array());
      $cat->setRencontre($data->rencontre);
      $cat->setTexte($data->texte);
      
      // create the product
      if($compteRendu->creerCompteRendu($cat))
      {
...
function SupprimerCompteRendu()
{
  try
  {
    ...
    if($compteRendu->supprimerCompteRendu($data->id))
    {
      // set response code - 200 ok
      http_response_code(200);
...
function ModifierCompteRendu()
{
  try
  {
    ...
    // create the product
    if($compteRendu->modifierCompteRendu($cat))
    {
      // set response code - 201 created
      http_response_code(201);

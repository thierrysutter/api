...
include_once '../dao/DaoArticle.php';
...
function AjouterArticle()
{
  try
  {
    ...
    if(!empty($data->titre))
    {
      ...
      $cat->setDateParution($data->dateParution);
      $cat->setAuteur($data->auteur);
      
      // create the product
      if($article->creerArticle($cat))
      {
...
function SupprimerArticle()
{
  try
  {
    ...
    if($article->supprimerArticle($data->id))
    {
      // set response code - 200 ok
      http_response_code(200);
...
function ModifierArticle()
{
  try
  {
    ...
    // create the product
    if($article->modifierArticle($cat))
    {
      // set response code - 201 created
      http_response_code(201);
